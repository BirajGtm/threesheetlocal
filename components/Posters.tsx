      ...
      <div className="container mx-auto px-4 md:px-10">
        ...
        <div className="flex flex-col md:flex-row gap-10 md:gap-20">
          ...
          <motion.div style={{ y: y1 }} className="flex-1 flex flex-col gap-20 md:pt-20">
            {leftColumn.map((item) => (
              <Card key={item.id} item={item} onClick={() => setSelectedImage(item.image)} />
            ))}
          </motion.div>
...
function Card({ item, onClick }: { item: typeof posterItems[0], onClick: () => void }) {
  return (
    // 1. Center the card in the column and constrain width
    <div className="w-full flex justify-center">
      <div
        className="group relative w-full max-w-[550px] cursor-zoom-in"
        onClick={onClick}
      >
